boolean bossgameStarted = false; // ← ゲームが開始されたかどうか
  ...
  if (!bossgameStarted) {
    ...
    textSize(80);
     ...
     if (elapsed < 1000) {
      ...
      bossgameStarted = true; // カウントダウン終了→ゲーム開始
    }
...
void handlePlayerMovement() {
  float nextX = playerbX;
  float nextY = playerbY;
...
// ボスとの接触チェック 
    ...
    boolean blockedByBoss = isOverlap(nextX, nextY, playerW, playerH, bossbX, bossY, bossW, bossH);
    ...
    if (!blockedByBoss) {
      ...
      if (atLeftEdge) {
        // 左端とボスに挟まれたら右に強制押し戻し
        playerbX = constrain(playerbX + 50, 0, width - playerW);
        isEdgePushed = true;
...
void updateBossMovement() {
  ...
  if (!isOverlap(nextX, nextY, bossW, bossH, playerbX, playerbY, playerW, playerH)) {
    // 重ならなければ移動
    bossbX = nextX;
    bossY = nextY;
  } else {
...
void handleBossKey() {
  ...
  if (!bossgameStarted && keyCode == ENTER) {
